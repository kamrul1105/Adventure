...
async function startIntro() {
  await sleep(300);
  await typewrite(introTitle, 'Hey Jalali ❤️', 70);
  await sleep(400);
...
function onNoClick() {
  ...
  btnYes.style.fontSize  = (1.5 + noClickCount * 0.3) + 'rem';
...
/** Crossfade from the playful track to the romantic one */
function switchToRomanticMusic() {
  if (!musicPlaying) return; // don't start music if she hasn't enabled it

  romanticMusic.volume = 0;
  romanticMusic.play().catch(() => {});
  currentTrack = romanticMusic;
  let v = bgMusic.volume;
  const fade = setInterval(() => {
    v -= 0.05;
    bgMusic.volume = Math.max(0, v);
    romanticMusic.volume = Math.min(0.35, romanticMusic.volume + 0.05);
    if (v <= 0) {
      clearInterval(fade);
      bgMusic.pause();
